...
export default function CustomerLoginPage() {
    ...
    try {
      if (isRegister) {
        ...
        const { data, error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            data: { full_name: fullName.trim(), store_name: storeName.trim(), phone, address }
          }
        })
        ...
        alert('Đăng ký tài khoản sỉ thành công!')
        router.push('/')
      } else {
        // ĐĂNG NHẬP
        ...
        alert('Đăng nhập thành công! Hệ thống đã đồng bộ giỏ hàng của bạn.')
        router.push('/')
      }
    } catch (err: any) {
      alert(`Lỗi xác thực: ${err.message || 'Không thể thao tác'}`)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSaveDefaultInfo = (e: React.FormEvent) => {
    ...
    alert('Đã lưu thông tin quán & địa chỉ giao hàng mặc định!')
  }
